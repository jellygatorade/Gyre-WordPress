...
function prevent_delete_if_acf_image_field_usage( $attachment_id ) {
    ...
    if (count($query->posts) === 0) {
        ncma_artwork_write_log("The attachment that was requested to be deleted is included in " . count($query->posts) . " posts, so it will be deleted.");
    ...
    // If 1 or more posts were found in the meta_query
        ...
        ncma_artwork_write_log("The attachment that was requested to be deleted is included in " . count($query->posts) . " posts.");
        ncma_artwork_write_log('It is currently attached as an image to:');

        // Loop through meta query results to log each discovered post containing the attachment
        for ($i = 0; $i < count($query->posts); $i++)  {
            ncma_artwork_write_log($query->posts[$i]->ID . ' - ' . $query->posts[$i]->post_title);
        }
...
function ncma_artwork_write_log( $log ) {
    if ( true === WP_DEBUG ) {
